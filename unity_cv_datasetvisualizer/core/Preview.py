...
def preview_app(args):
    ...
    with st.sidebar:
        # Add logo
        st.image(UI.get_docs_path("unity_logo.png", as_str=True))
        ...
        if instance_list is not None and len(instance_list) > 1:
            ...
            if new_instance != chosen_instance:
                UI.set_selected_instance(new_instance)
...
if __name__ == "__main__":
    favicon = Image.open(UI.get_docs_path("favicon.ico", as_str=True))
    st.set_page_config(
        page_title="Unity Dataset Visualizer",
        layout="centered", page_icon=favicon,
